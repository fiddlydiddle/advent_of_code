...
fn part2(battery_banks: &Vec<&str>, num_digits: u32) -> i64 {
    ...
    for battery_bank in battery_banks {
        ...
        while digits_remaining > 0 {
            ...
            let window_end = battery_bank.len() - (digits_remaining as usize) + 1;
            ...
            let exponent: u32 = digits_remaining - 1;
            bank_value += max_significant_digit * 10_i64.pow(exponent);
            digits_remaining -= 1;
        }
...
fn main() {
    ...
    let example_ranges: Vec<&str> = example_input.trim().split('\n').collect();
    let input = fs::read_to_string("input.txt")
        .expect("Something went wrong reading the file");
    let ranges: Vec<&str> = input.trim().split('\n').collect();

    // Part 1 Example
    let part1_example_result = part2(&example_ranges, 2);
    println!("Part 1 (example): {}", part1_example_result);

    // # Part 1
    let part1_result = part2(&ranges, 2);
    println!("Part 1: {}", part1_result);

    // Part 2 Example
    let part2_example_result = part2(&example_ranges, 12);
    println!("Part 2 (example): {}", part2_example_result);

    // Part 2
    let part2_result = part2(&ranges, 12);
